...
exportedPackageDefinitions
	...
	exportedClassDefinitions
		...
		(
		exportedConstantDefinitions
			MockError_MethodMockAlreadyCalled;
			MockError_MockParameterValidationFailed;
		exportedMethodDefinitions
			isMockedObject;
		)
...
begin
	...
	if zMockedReturnValues.size() <> 0 then
		SystemException.raise_(JadeMockingFramework.MockError_MockParameterValidationFailed, "Fixed return value has already been specified");
	endif;
	// validation - make sure the method mock has not already been called
	if zMockCallHistories.size() <> 0 then
		SystemException.raise_(JadeMockingFramework.MockError_MethodMockAlreadyCalled, "Method mock has already been called");
	endif;
...
begin
	...
	if zMockedParameters.size() <> 0 then
		SystemException.raise_(JadeMockingFramework.MockError_MockParameterValidationFailed, "Fixed parameter values have already been specified");
	endif;
	// validation - make sure the method mock has not already been called
	if zMockCallHistories.size() <> 0 then
		SystemException.raise_(JadeMockingFramework.MockError_MethodMockAlreadyCalled, "Method mock has already been called");
